...
class PFBExporter:
    ...
    def filter_attributes(self, is_black_list=False):
        ...
        if any(f for f in invalid_fields.values()):
            raise RuntimeError(f'Invalid fields in config: {({k:v for k,v in invalid_fields.items() if v})}')
